...
- `cursorNext` provides a cursor to fetch next page of tiles
- `cursorPrevious` provides a cursor to fetch previous page of tiles, but that would be empty since we are on the first page
- We received 25 tiles (the default limit)

## Customizing the Limit

You can control how many items you want per page using the `limit` parameter:

### Fetch only 5 tiles per page:
```bash
curl \
  -X GET \
  -H 'Authorization: Bearer sk_Guei2HJgU...' \
...
Response will include second page of tiles:
...
Similarly, when paginating backwards, last page will have `cursorPrevious` set to `null`, number of items will be 0, and `cursorNext` will still contain a valid cursor to go in opposite direction.
