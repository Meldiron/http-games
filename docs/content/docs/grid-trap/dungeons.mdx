...
In a game of GridTrap, your goal is to escape from a dungeon by avoiding trapdoors.

To start a new game, send the following cURL request to create a new dungeon:
...
<Accordions type="single">
  <Accordion title="Response">
    ...
    Internal error, likely due to dungeon generation. Hopefully never happens, and if it does, re-sending the same request likely helps.
    ...
    The dungeon has been created and you can play it.
...
Getting details about a dungeon can be done right after creating it, or anytime in the future. It is mostly useful to check the status of the game.
...
<Accordions type="single">
  <Accordion title="Response">
    ...
    The dungeon exists and you are given details about it.
...
You can get a list of all dungeons you have created. This can be useful in many scenarios:
...
To fetch a list of dungeons, send the following request:
...
<Accordions type="single">
  <Accordion title="Response">
    ...
    A list of dungeons is returned.
...
As an example of a more complex search, if you wanted to get all unfinished dungeons of size 4x4 or 7x7 that are not hardcore, you could send the following request:
...
This endpoint also supports pagination using `limit` and `cursor` parameters. Learn more about it in the [pagination documentation](https://http-games.almostapps.eu/docs/basics/pagination).